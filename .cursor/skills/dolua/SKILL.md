...
6. 「错误与返回约定」：**诊断先看这一节。** 有的失败返回 `false`，有的抛错，有的 `nil, err` / `nil, err_code, err_msg` / 回调 `ev.code`。文案和数字码以该节全表为准，不要猜。对照「可能原因」再改脚本或查驻网/配额/对象是否还活着
...
5. 用户报失败：先打开对应模块「错误与返回约定」，用返回文本 / 错误码 / 抛错摘要对表，不要凭其它模组生态猜。

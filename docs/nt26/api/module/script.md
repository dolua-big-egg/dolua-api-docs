...
纯函数模块。查询当前正在跑的脚本副本（boot bundle），以及清单。底层按 A/B 两槽设计，**当前产品配置未启用双槽切换**（本版本可用空间偏紧；日后 8+8 / CAT4 才可能打开），见 [DoLua 核心 · 运行](../../core/runtime.md#2-脚本副本a--b-两槽)。
...
**当前 NT26（含 NT26 PRO）未启用双槽切换。** 内部配额不够同时留两份完整包。把设备当成一份正在跑的脚本即可；不要按「空闲槽试运行 → 确认 / 回滚」写业务。日后 **8+8** 或 **CAT4** 版本才可能打开，见 [核心 · 运行第 2 节](../../core/runtime.md#2-脚本副本a--b-两槽)。

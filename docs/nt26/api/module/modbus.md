...
一条规则连续抠 **最多约 20 个 16 位保持寄存器**（40 字节）。一次 `analyze` 可以写多条规则，超过 20 个就拆开，详见 [第 12 节](#12-资源上限与生命周期)。
...
`len` 必须是该类型**单位宽度**的整数倍，合计不超过 **40** 字节（20 个 16 位寄存器），否则整次 `analyze` 失败（`nil`）。
...
if vals == nil then
    -- 规则越界、len 不是类型宽度的整数倍、某条不是 4 元组
else
    -- vals[1]、vals[2] 是 number
end

-- 一条规则连续抠 20 个 UINT16（40 字节），vals[1] 是 20 个数的表
...
| `len` | 取几个字节；必须是该 `mode` 单位宽度的整数倍，合计 `1`～`40` |
| `mode` | 见 [4.1](#41-数据类型规则第-3-项-mode) |
| `endian` | 见 [4.2](#42-字节序规则第-4-项-endian) |

`start + len - 1` 不能超出 `data` 长度。`len` 范围 `1`～`40`，且必须是类型宽度的整数倍。一条规则连续抠 16 位寄存器最多 **20 个**：`{4, 40, 3, 1}`（数据从应答第 4 字节起，UINT16 大端）。更多寄存器拆成多条，每条仍不超过 40 字节。

多条规则互不影响，可以重叠、可以跳过中间字节（例如跳过从站、功能码、CRC）。

任一条不是 4 元组、越界、`len` 不是类型宽度的整数倍、类型和长度不配：整次返回 `nil`。失败原因只打到设备日志，Lua 侧看不到字符串。
...
| 实例 | 无 userdata |
| `analyze` 单条规则 | 最多 **40 字节** |
| 一次连续抠 16 位寄存器 | **20 个**（`UINT16` / `INT16`，`len=40`） |
...
| 类型 | `mode` | 一条规则最多 |
| --- | --- | --- |
| UINT16 / INT16（保持 / 输入寄存器） | `3` / `4` | **20** |
| UINT32 / INT32 / FLOAT32 | `5` / `6` / `9` | 10 |
| UINT64 / INT64 | `7` / `8` | 5 |
| UINT8 / INT8 | `1` / `2` | 40 |

写法：`{4, 40, 3, 1}` 就是从应答第 4 字节起连续 20 个 UINT16 大端，该条结果是 20 个数的表。

一次 `analyze` **可以写多条规则**，每条各自不超过 40 字节。要一次解超过 20 个 16 位寄存器，拆成两条即可，例如前 20 个 `{4, 40, 3, 1}`，接着 10 个 `{44, 20, 3, 1}`。整帧仍须已经完整收到；本模块不替你拼包。

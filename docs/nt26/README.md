...
本型号的文档入口。新手先看 **[开始使用](../get-started/get-started.md)**（装工具、第一份工程；下载、工程形态、工具、调度已拆成同目录其它页）。再读 **doLua 核心**（是什么、怎么跑、消息与调度、资源上限），再按模块查 API。硬件全 IO 在 `hardware/`；应用 AT 在 `at/`；示例按工程用途分在 `examples/nt26` 的 `started` / `os` / `peripherals` / `network` / `storage` / `module` / `apps`。分类不必一一对应，用下面的索引跳转即可。

| | 路径 |
| --- | --- |
| **开始使用**（安装、第一份工程；同目录还有下载 / 工程 / 工具 / 调度） | [../get-started/get-started.md](../get-started/get-started.md) |

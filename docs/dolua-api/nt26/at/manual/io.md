...
| `AT+IOPUL` | 单脉冲 |
| `AT+IOSEQ` | CSV 波形 |
| `AT+ADC` | 读电压（mV）或内部温度（°C）；数字通道或 `"ch_*"` 名 |
...
波形 `IOSEQ` 捕获整段参数（CSV）。形如：id、起始电平、若干保持毫秒、结束电平。保持时间按内部节拍量化成字节，整段载荷最长 **256**。测试 `AT+IOSEQ=?` 看帮助。跑着时其它 IO 操作可能返回 **136**。
...
换模块时用字符串名，不要写死数字。`chN` 与芯片 ADC 通道编号一致（`ch1` 就是 VBAT）。

| 名字 | 含义 | 应答第二段 |
| --- | --- | --- |
| `"ch_thermal"` / `"ch0"` | 内部温度 | 摄氏度整数 |
| `"ch_vbat"` / `"ch1"` | 电池电压 | mV |
| `"ch_aio4"` / `"ch2"` | AIO4 | mV |
| `"ch_aio3"` / `"ch3"` | AIO3 | mV |
| `"ch_aio2"` / `"ch4"` | AIO2 | mV |
...
名字大小写不敏感，必须加双引号。失败 **105** / **138**。无查询形态。
...
先确认该 id 在硬件落盘表里。`IOCFG` 只落盘，重启后按配置生效；未初始化就 SET/GET 会 **125**。
...
| 2.0.0 | 2026-09-21 | 删除 `AT+IOINIT`（不再对外暴露） |
| 2.1.0 | 2026-09-21 | `AT+ADC` 增加 `"ch_thermal"` / `"ch_vbat"` / `"ch_aio1"`… 字符串通道 |

...
纯函数模块。查询当前正在跑的脚本副本（boot bundle），以及清单。底层按 A/B 两槽设计，**当前产品配置未启用双槽切换**（本版本可用空间偏紧；日后 8+8 / CAT4 才可能打开），见 [doLua 核心 · 运行](../../../../dolua-core/runtime.md#2-脚本副本a--b-两槽)。
...
日常用 `script.info()` 读本工程的 `version` / `build` / `bundle_id`。`script.switch`、`script.rollback` 是 **预留接口，暂未部署，请不要使用**。当前版本也不要把试运行 / 切槽当成已开通的产品路径。
...
脚本底层按 **A、B 两槽** 存放。开机选中并真正跑起来的那一份叫 **boot bundle**。

**当前 NT26（含 NT26 PRO）未启用双槽切换。** 内部配额不够同时留两份完整包。把设备当成一份正在跑的脚本即可；不要按「空闲槽试运行 → 确认 / 回滚」写业务。日后 **8+8** 或 **CAT4** 版本才可能打开，见 [核心 · 运行第 2 节](../../../../dolua-core/runtime.md#2-脚本副本a--b-两槽)。

| 需求 | 调用 | 当前版本 |
| --- | --- | --- |
| 当前这份脚本的版本 | `script.info()` | 可用 |
| 看某一槽清单 | `script.info("A")` / `script.info("B")` | 可查询；不要据此做切槽 |
| trial 跑通，升成正式副本 | `script.confirm()` | 切槽未启用时不要当升级路径 |

`confirm` 只改副本状态，**不会立刻重启**。trial 由下载/AT 选槽进入；脚本侧只负责跑起来之后 `confirm`。当前配置下不要走这条路径。
...
当前配置 **未启用双槽切换**，不要按上图做升级或回滚。打开之后的语义：
...
| 看另一槽有没有包 | 可 `pcall(script.info, "B")`；当前未切槽，不要当升级依据 |
| 新包 trial 跑通 | **当前不要用。** 切槽未启用 |
...
| 1.2.0 | 2026-09-12 | 标明双槽切换为底层设计、当前配置未启用；8+8 / CAT4 才可能打开 |

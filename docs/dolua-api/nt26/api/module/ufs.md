...
脚本越大，ufs/ublob 能用的就越少。底层两槽的脚本若都落盘会一起计入脚本区；**当前未启用双槽切换**，不要按两份完整包预留。脚本升级变大时 `limit` 会下降，可能把已有文件挤到写不下。清空间：删本模块文件或 `ublob.remove`，两边的 `usage().used` 会一起下降。整图见 [资源](../../resources/README.md)、[存储怎么分](../../resources/storage.md)。

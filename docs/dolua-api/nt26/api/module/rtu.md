...
每个数字指向哪里、子通道上限、查询回显怎么规范化，以 AT [路由串](../../at/manual/route.md) 为准。下面只列常用写法：

| 写法 | 含义 |
| --- | --- |
| `"1"` / `"2"` / `"3"` / `"4"` | RTU 任务通道 1～4 |
| 用竖线并选，例如 `"1\|2"` | 同时发到通道 1 和 2 |
| `"1[1:2]"` | 通道 1 的 MQTT 发布槽 1 和 2（该路是 SOCK 时子通道被忽略） |
...
| 1.1.3 | 2026-09-07 | 第 8 节路由串改以 AT [route.md](../../at/route.md) 为完整对照 |
| 1.1.4 | 2026-09-19 | AT 路由串链接改到 `at/manual/route.md` |

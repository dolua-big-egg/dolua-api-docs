...
运行时调度：协作式任务、让出延时、信箱、显式队列、软件定时器。没有对象。脚本里的「多任务」是同一虚拟机里的协程，不是操作系统抢占线程。整图（虚拟机如何启动、回调为何不在中断里）见 [doLua 核心](../../../../dolua-core/README.md)；堆和 Flash 见 [资源](../../resources/README.md)。
...
| 1.1.1 | 2026-09-12 | 文首链到 doLua 核心（运行机制与调度整图） |

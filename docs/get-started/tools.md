...
![checked-files](img/checked-files.webp)
...
![new-in-zone](img/new-in-zone.webp)

![new-module](img/new-module.webp)

但是内置文件系统和其他文件区则可以新建任意文件

![new-other](img/new-other.webp)

也可以直接拖动文件，放到工程文件中，则文件会先自动放到其他文件中

![drag-in-file](img/drag-in-file.webp)

![dropped-to-other](img/dropped-to-other.webp)

### 移动文件

右键文件，则可以移动到对应的分区

![move-zone](img/move-zone.webp)

因为有规则限制，有些移动必须是指定后缀的，比如一定得 `.lua` 的移动，才有移动到模块和主文件的选项。

![to-module](img/to-module.webp)

### 文件转工程

这里讲的是，如果你有一份代码，要怎么把他变成工程，很简单，你只需要构建工程信息，先在工作目录下新建一个文件夹，把文件夹名字改成你要的工程名，比如这里是`my-project`，然后在里面放入你的代码，有什么就放什么，只有main 也行。

![make-project-folder](img/make-project-folder.webp)

此时目录树下还无法显示该文件夹。

![hidden-folder](img/hidden-folder.webp)

此时需要打开对没有工程描述文件的文件夹的显示

![show-folders](img/show-folders.webp)

当打开显示后，可以看到这个文件夹了，右键他，构建工程描述文件

![create-luaproj](img/create-luaproj.webp)

此时他在目录树中，就由文件夹转变成了工程

![folder-as-project](img/folder-as-project.webp)

### 例程中心

不要从零手写第一份脚本去试接口，直接 **一键下载** 不同的 demo 来测试功能，可以用搜索功能来搜索例程哦。

![demo-center](img/demo-center.webp)
...
![download-port](img/download-port.webp)

![download-run](img/download-run.webp)

### 打包与导出

选中工程后点击导出，即可导出 `.luapk` 这个是可以直接给设备下载的固件，也可以拖动到工程目录进行还原。注意打包文件只会打包选中的文件，也就是`工程文件`中没有被选中的文件不会参与打包到`.luapk`。

![export-project](img/export-project.webp)
...
![mcp-register](img/mcp-register.webp)

![mcp-refresh](img/mcp-refresh.webp)

引用我们的 skill

![use-skill](img/use-skill.webp)

写个简单的需求试试，这里没有详细的描述，让 AI 自由发挥

![ask-ai](img/ask-ai.webp)

等待一段时间后，AI输出如下

![ai-done](img/ai-done.webp)

...
脚本越大，ufs/ublob 能用的就越少。A/B 两槽的脚本都计入脚本区。脚本升级变大时 `limit` 会下降，可能把已有文件挤到写不下。清空间：删本模块文件或 `ublob.remove`，两边的 `usage().used` 会一起下降。整图见 [可用资源](../../core/resources.md)、[存储怎么分](../../core/storage.md)。

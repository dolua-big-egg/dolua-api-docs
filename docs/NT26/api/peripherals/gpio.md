...
**NT26-PRO** 使用 **F6E0** 与 **F6D0** 两种封装。F6D0 固件不另开一张表，GPIO↔PIN **与 F6E0 完全相同**，做板与脚本都按 [1.2](#12-nt26-pro-map) 对脚。丝印脚名、默认外设和全部复用见 [NT26-PRO 全 IO 表](../../hardware/pro.md)。F6B0 是另一封装，PIN 不同，见 [1.3](#13-nt26-f6b0-映射表)。

...
打开时完成编号解析。之后 `set` / `get` / `tog` / `seq` 走对象上的快速句柄，不再反复查表。
...
两种写法打开的是同一条物理脚时，后续 `pins()` 会给出互相对应的 GPIO 编号和 pin 序号。非法编号在 `open` 时直接抛错。

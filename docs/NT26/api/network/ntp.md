...
问询失败、写钟失败、锁失败：一律单独 `nil`，**无错误码**、无第二返回值。分不清是没问到还是没写成。

**何时 `nil`（无错误码）**

- 未驻网、DNS 失败、UDP 失败、超时（默认整次 6000 ms）
- 问询成功但第五参 `auto_set` 为真、写本机钟失败
- 内部互斥锁创建或获取失败
- 问询返回空串
...
| 摘要 | 可能原因 |
| --- | --- |
| `invalid port` | `port` 不在 `0`～`65535` |
| `invalid timeout` | `timeout_ms <= 0` |

`server` / `port` / `timeout_ms` / `tz_min` 类型不对，或 `auto_set` 既不是 boolean 也不是整数：Lua 标准类型错。缺参不抛，走默认服务器/端口/超时。
...
| 1.1.0 | 2026-09-05 | 补全错误与返回约定：抛错摘要、无错误码的 `nil` 场景与可能原因 |

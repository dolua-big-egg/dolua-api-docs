...
本型号的 Lua API 与示例入口。文档按主题分在 `api/module`、`api/network`、`api/peripherals`、`api/rtu_config`；硬件全 IO 在 `hardware/`；示例按工程用途分在 `examples/NT26` 的 `started` / `os` / `peripherals` / `network` / `storage` / `module` / `apps`。分类不必一一对应，用下面的索引跳转即可。

--[[ flash — SPI 外挂 W25Qxx，整片挂 LittleFS（littlefs demo）
  ...
  不固定 16MB：sfud 读到容量后，offset=0、size=整片（按擦除粒度向下对齐），全部交给 lfs。
  本 demo 只挂 LittleFS，不混用 FlashDB。
...
function M.init()
    ...
    -- lfs.mount：offset=0、size=整片；on_event 打格式化/挂载进度
    local fs_cfg = {
        name = cfg.fs.name,
        offset = off,
        size = size,
